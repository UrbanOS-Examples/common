...
resource "aws_security_group" "workers" {
  ...
  description = "Security group for all nodes in the cluster."
  vpc_id      = module.vpc.vpc_id
  #JARRED tags        = "${map("Name", "${terraform.workspace}-eks_worker_sg", "kubernetes.io/cluster/${terraform.workspace}", "owned")}"
}
...
resource "aws_security_group" "private_workers" {
  ...
  vpc_id      = module.vpc.vpc_id
  #JARRED tags        = "${map("Name", "${terraform.workspace}-private-eks_worker_sg", "kubernetes.io/cluster/${terraform.workspace}-private", "owned")}"
}
...
resource "aws_security_group" "public_workers" {
  ...
  vpc_id      = module.vpc.vpc_id
  #JARRED tags        = "${map("Name", "${terraform.workspace}-public-eks_worker_sg", "kubernetes.io/cluster/${terraform.workspace}-public", "owned")}"
}
