...
function set_session {
    local role_json=$(aws sts assume-role --role-arn arn:aws:iam::073132350570:role/admin_role --role-session-name JenkinsWaitForInfra)
    ...
    export AWS_SECURITY_TOKEN=$(echo $role_json     | jq -r '.Credentials.SessionToken')
}
