resource "aws_vpc_peering_connection" "env_to_alm" {
  ...
  peer_vpc_id   = data.terraform_remote_state.alm_remote_state.outputs.vpc_id
...
resource "aws_route" "public_peer_env_to_alm" {
  ...
  destination_cidr_block    = data.terraform_remote_state.alm_remote_state.outputs.vpc_cidr_block
  vpc_peering_connection_id = aws_vpc_peering_connection.env_to_alm.id
}
...
resource "aws_route" "public_peer_alm_to_env" {
  ...
  route_table_id = element(
    data.terraform_remote_state.alm_remote_state.outputs.public_route_table_ids,
    0,
  )
...
resource "aws_route" "private_peer_alm_to_env" {
  ...
  route_table_id = element(
    data.terraform_remote_state.alm_remote_state.outputs.private_route_table_ids,
    0,
  )
  destination_cidr_block    = module.vpc.vpc_cidr_block
  vpc_peering_connection_id = aws_vpc_peering_connection.env_to_alm.id
