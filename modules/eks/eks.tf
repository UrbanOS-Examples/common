module "eks-cluster" {
  ...
  worker_additional_security_group_ids = ["${var.chatter_sg_arn}"]
...
resource "local_file" "aws_props" {
    content = <<EOF
aws:
  publicSubnets: ${jsonencode(var.public_subnets)}
  allowWebTrafficSecurityGroup: ${var.allow_all_sg_arn}
  certificateArn: "${var.tls_cert_arn}"
EOF
...
helm upgrade --install cluster-infra ${path.module}/helm/cluster-infra \
    ...
    --set externalDns.args."domain\-filter"="${var.internal_public_hosted_zone_name}" \
...
resource "null_resource" "tear_down_load_balancers" {
  provisioner "local-exec" {
    ...
    ${path.module}/../scripts/destroy_albs_created_via_kubernetes.sh ${var.vpc_id} ${var.region} ${var.role_arn}
  EOF
  }
}
