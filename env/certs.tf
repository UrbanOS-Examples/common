module "tls_certificate" {
  ...
  domain_name               = "${coalesce(var.tls_certificate_dns_name_override, local.public_hosted_zone_name)}"
  subject_alternative_names = ["*.${coalesce(var.tls_certificate_dns_name_override, local.public_hosted_zone_name)}"]
  hosted_zone_id            = "${coalesce(var.tls_certificate_public_hosted_zone_id_override, aws_route53_zone.public_hosted_zone.zone_id)}"
...
module "root_tls_certificate" {
  ...
  domain_name               = "${var.root_dns_zone}"
  subject_alternative_names = ["*.${var.root_dns_zone}"]
  hosted_zone_id            = "${aws_route53_zone.root_public_hosted_zone.zone_id}"
  validation_record_ttl     = "60"
}
