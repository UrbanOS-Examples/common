...
resource "aws_db_instance" "ambari_db" {
  ...
  vpc_security_group_ids  = ["${aws_security_group.ambari_security_group.id}", "${aws_security_group.postgres_allow_cloudbreak.id}"]
  ...
  apply_immediately       = "${var.ambari_db_apply_immediately}"
...
variable "ambari_db_apply_immediately" {
  description = "Should changes to the Ambari DB be applied immediately?"
  default = false
}
